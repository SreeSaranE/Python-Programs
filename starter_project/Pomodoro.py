...
class Pomodoro:
    ...
    def countdown(self):
        if self.load_time > 0:
            ...
            formatted_time = f"{mins:02}:{sec:02}"
            self.time_display.configure(text=f"Time: {formatted_time}")
            self.load_time -= 1
            self.window.after(1000, self.countdown)
        else:
            self.time_display.configure(text="TimeOut")
